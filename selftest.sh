...
runuser -u mw5stream -- test -r "${CONFIG_FILE}"
runuser -u mw5stream -- test -r "${SCHEDULE_FILE}"

# Can mw5stream write into runtime dirs?
runuser -u mw5stream -- touch "${BASE_DIR}/state/.perm_test"
runuser -u mw5stream -- rm -f "${BASE_DIR}/state/.perm_test"
...
if [[ "${do_probe}" == "1" ]]; then
  ...
  runuser -u mw5stream -- \
    "${TIMEOUT_BIN}" "${PROBE_TIMEOUT_SEC}" "${FFPROBE_BIN}" \
    -v error \
    -rtsp_transport tcp \
    -stimeout "${PROBE_STIMEOUT_US}" \
    "rtsp://${CAM_IP}:554/stream=1" \
    >/dev/null
